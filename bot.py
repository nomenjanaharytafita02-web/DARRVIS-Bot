...
WEBHOOK_URL = os.getenv("WEBHOOK_URL") # This will be set by Render
...
async def aide(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_message = (
        ...
        "5. **Lecture de fichiers** : Envoyez-me un fichier PDF ou Word, et je pourrai en extraire le texte et répondre à vos questions.\n"
...
async def init_ptb_application():
    """Initializes the python-telegram-bot Application instance."""
    ...
    application_ptb.add_handler(MessageHandler(filters.Document.PDF | filters.Document.DOCX, handle_document))

    await application_ptb.initialize()
    if WEBHOOK_URL:
        await application_ptb.bot.set_webhook(url=f"{WEBHOOK_URL}/{TELEGRAM_TOKEN}")
...
async def telegram_webhook():
    if application_ptb is None:
        ...
        return "Error: Telegram Application not initialized", 500

    update = Update.de_json(request.get_json(force=True), application_ptb.bot)
    await application_ptb.process_update(update)
    return "ok"


if __name__ == "__main__":
    # Initialize the PTB application in a separate asyncio task
    loop = asyncio.get_event_loop()
    loop.create_task(init_ptb_application())

    if WEBHOOK_URL:
        logger.info(f"Starting Flask app for webhook on port {PORT}")
        # For Render.com, Gunicorn will be used to run this Flask app.
        # The `app.run()` below is for local testing.
        app.run(host="0.0.0.0", port=PORT)
    else:
        logger.info("WEBHOOK_URL not set, falling back to polling mode.")
        # If no webhook URL, run polling directly in the main thread
        # This will block, so it\'s only for local testing without webhook
        # Ensure application_ptb is initialized before calling run_polling
        loop.run_until_complete(init_ptb_application())
        application_ptb.run_polling(allowed_updates=Update.ALL_TYPES)

    # Keep the event loop running for the bot\'s background tasks
    loop.run_forever()
